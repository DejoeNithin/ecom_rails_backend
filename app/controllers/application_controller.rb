class ApplicationController < ActionController::Base
    ...
    def validate_auth_token
        if request.headers["Authorization"].blank?
            render json: {'status_code': 401, message: '1 - Missing or wrong authentication token'}, status: 403 and return
        end 
        ...
        if request.headers["Authorization"].to_s != @token.token.to_s
            render json: {'status_code': 401, message: '2 - Missing or wrong authentication token'}, status: 403 and return
        end
